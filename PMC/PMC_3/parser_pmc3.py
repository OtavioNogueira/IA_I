...
# Parse train
matches = re.findall(r't\s*=\s*(\d+)\s+([0-9.]+)', raw_train)
for m in matches:
    t = int(m[0])
    v = float(m[1])
    data[t] = v
...
# Parse test
matches = re.findall(r't\s*=\s*(\d+)\s+([0-9.]+)', raw_test)
for m in matches:
    t = int(m[0])
    v = float(m[1])
    data[t] = v
...
with open('dados_serie.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['t', 'f(t)'])
    for t, v in sorted_data:
        writer.writerow([t, v])

print(f"Parsed {len(sorted_data)} records into dados_serie.csv")
